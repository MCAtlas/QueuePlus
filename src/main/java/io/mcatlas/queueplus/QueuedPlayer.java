...
public class QueuedPlayer implements ForwardingAudience.Single {
    ...
    public Player player() {
        return Main.instance().proxy().getPlayer(this.uuid).orElse(null);
    }
    ...
    private @NotNull Priority calculatePriority() {
        ...
        for (Priority priority : Main.instance().config().priorities()) {
    ...
    public @NotNull Audience audience() {
        return Main.instance().proxy().getPlayer(this.uuid).map(player -> (Audience) player).orElse(Audience.empty());
    }
    ...
    public void loadData() {
        if (!dataLoaded) {
            dataLoaded = true;
            Main.instance().storage().loadPlayer(this);
        }
    }
