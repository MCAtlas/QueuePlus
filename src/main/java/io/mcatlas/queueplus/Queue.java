...
public class Queue {
    ...
    public Queue(RegisteredServer server, Main plugin) {
        this.server = server;
    ...
    public void sendNext() {
        ...
        player.createConnectionRequest(server).connect().thenAccept(result -> {
            ...
            Main.warn("An exception occurred while trying to send " + player.getUsername() + " to " + formattedName + ":");
            e.printStackTrace();
            player.sendMessage(Component.text("Unable to connect you to " + formattedName + ".", NamedTextColor.RED));
        ...
        if (player.queue() != null) {
            if (player.queue().equals(this)) {
                ...
                if (!confirmation) {
                    ...
                    Main.log(player.name() + " has been removed from the queue, because they queued for another.");
